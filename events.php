<?php
// Страница мероприятия (Все пользователи)
                ...
                <div class="container" style="margin-top:1%; margin-bottom:1%;">
                    <div class="clearfix">
                        <div class="md-col md-col-12 lg-col-12 p2">
                            ...
                            // Запрос для получения данных мероприятий и связанных фотографий
                            $query = "
                                SELECT `events`.`id_events`, `events`.`caption`,`events`.`description`, `events`.`datep`,
                                `events`.`statusevents`,`uphotoevent`.`id_uphotoevent`,
                                `uphotoevent`.`uphotoevent`,`uphotoevent`.`id_events`
                                FROM `events`
                                LEFT JOIN `uphotoevent` ON `events`.`id_events` = `uphotoevent`.`id_events`
                                WHERE `events`.`id_events` = $idevents";
                            ...
                            if ($result && $result->num_rows > 0) {
                                ...
                                do {
                                    if (!empty($row['uphotoevent'])) {
                                        $photos[] = 'data:image/jpeg;base64,' . base64_encode($row['uphotoevent']);
                                    }
